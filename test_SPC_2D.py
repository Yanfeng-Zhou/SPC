...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--path_model', default='/mnt/data1/SPC/pretrained_model/P-CT/best_result1_Jc_0.6808.pth')
    ...
    parser.add_argument('-n', '--network', default='unet')
    ...
    for i, subject in enumerate(dataset_val.dataset_1):
        ...
        with torch.no_grad():
            ...
            for data in dataloaders['test']:
                ...
                B, C, H, W, D = inputs_test.shape
                inputs_test_2D = inputs_test.permute(0, 4, 1, 2, 3).reshape(B * D, C, H, W)
                location_test = data[tio.LOCATION]

                outputs_test = model(inputs_test_2D)
                outputs_test_re = outputs_test.reshape(B, D, cfg['NUM_CLASSES'], H, W).permute(0, 2, 3, 4, 1)
                aggregator.add_batch(outputs_test_re, location_test)
